...
@Component({selector: 'hrs-consulation-manage', templateUrl: './consulation-manage.component.html', styleUrls: ['./consulation-manage.component.css']})
export class ConsulationManageComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        document.querySelector('#stepper1').addEventListener('show.bs-stepper', (event : any) => {
            const url = this.location.path().replace(this.steps[event.detail.from], this.steps[event.detail.to]);
            this.location.go(url);
        });
    ...
    patientDetailsNextClick(patientdetails : any) { // Save data and get back the saved object with ID
        let cId = 1;
